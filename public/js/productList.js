$(document).ready(function(){
	...
	/* 控制首页是否展示 */
	$('.displayindex').on('switchChange.bootstrapSwitch', function(e, state) {
		var _onItems = parseInt($("#onItems").val()),
				$this = $(this),
				count = 8;

		if (_onItems > count) {
			layer.msg('只能选择'+count+'条在首页展示！请刷新！');
			return;
		}
	...
	$('.isSale').on('switchChange.bootstrapSwitch', function(e, state) {
		var $this = $(this);

		$.ajax({
	  		type: 'POST',
	  		url: '/admin/product/isSale',
	  		data: {'id': $this.data('id'), 'on': state},
	  		success: function(data){
	  			if (data.success === 1 && $this) {
					layer.msg('打折状态设置成功！');
					return;
	...
	$("#update").on('click', function(){
		...
		if (rows.length == 0) {
			layer.alert('请选择一个产品更新！', {icon: 0, title:'提示'});
		}else if(rows.length > 1){
			layer.alert('一次只能更新一个产品！', {icon: 0, title:'提示'});
		}else{
